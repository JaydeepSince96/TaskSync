...
export class TaskReminderController {
  ...
  testNotification: RequestHandler = async (req, res) => {
    try {
      const userId = getUserId(req);
      ...
      if (!['morning', 'evening', 'overdue'].includes(type)) {
        res.status(400).json({
          success: false,
          message: "Invalid notification type. Must be 'morning', 'evening', or 'overdue'"
        });
        return;
      }
      ...
      // Get user's first incomplete task for testing
      const Task = require('../models/task-model').default;
      ...
      if (!task) {
        res.status(404).json({
          success: false,
          message: "No incomplete tasks found for testing"
        });
        return;
      }

      // Send test notification
      const { sendTaskReminder } = require('../services/task-reminder-scheduler');
      
      const success = await scheduler['sendTaskReminder']({
        task,
        user,
        reminderType: type as 'morning' | 'evening' | 'overdue',
        daysUntilDue: scheduler['calculateDaysUntilDue'](task.dueDate)
      }, user.deviceTokens || []);
